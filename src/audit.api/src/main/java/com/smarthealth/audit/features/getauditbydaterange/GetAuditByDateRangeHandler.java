...
public class GetAuditByDateRangeHandler
        implements QueryHandler<GetAuditByDateRangeQuery, Page<AuditEntryDto>> {
    ...
    public Page<AuditEntryDto> handle(GetAuditByDateRangeQuery query) {
        return repository.findByOccurredAtBetween(query.from(), query.to(), query.pageable())
                .map(mapper::toDto);
    }
}
